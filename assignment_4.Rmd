...
author: |
  | Abe Dijkstra
  | abe.dijkstra@ziggo.nl
  | http://www.github.com/abedijkstra1
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW <- GrowthSW %>%
  mutate(treat = ifelse(revolutions == 0, "0 Revolutions", "More than 0 Revolutions"))
...
In this case, the obtained p-value of 0.06871 indicates that there is a moderate level of evidence against the null hypothesis. The conventional significance level often used is 0.05. Since the p-value (0.06871) is greater than the significance level (0.05), we do not have sufficient evidence to reject the null hypothesis. This means that based on the t-test results, we do not have a strong statistical evidence to that there is a significant difference in the mean growth rates between countries that experienced at least one revolution/episode of political instability and countries that did not.  
...
The 'rdgp60' variable means the value of GDP per capita in 1960, converted to 1960 US dollars. By including the variable 'rgdp60' in the model, we can account for the  economic conditions in 1960 when examining the relationship between the growth rate and the occurrence of revolutions. This variable helps to control for the baseline economic state of each country before analyzing the impact of revolutions on subsequent growth.
...
sumary_table <- models |>
  modelsummary(stars= TRUE, statistics = c("novs", "rsq"))
...
The main driver of economic growth can be identified by comparing the R-squared values across the models. The model with the highest R-squared provides the best explanation for economic growth, indicating that its included variables have a stronger relationship and account for more of the variability in economic growth. In this case model 4 has the highest R-squared value, meaning education has the highest contribution to economic growth
...
summary_table %>%
  kable() %>%
  kable_styling() %>%
  row_spec(which(summary_table$term == "treat"), background = "red", color = "white")
